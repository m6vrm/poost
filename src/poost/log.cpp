...
void log_print(const LogSettings &settings, LogLevel level, const char *file, int line,
    ...
    if (settings.use_color) {
        ...
        if (settings.print_file_line) {
            std::fprintf(settings.stream, "%s%-5s\x1b[0m \x1b[90m%s:%d:\x1b[0m ", color, label,
                         file, line);
        } else {
    ...
    va_list ap;
    va_start(ap, fmt);
    // todo: false positive (see https://github.com/llvm/llvm-project/issues/40656)
    std::vfprintf(settings.stream, fmt, ap); // NOLINT(clang-analyzer-valist.Uninitialized)
    va_end(ap);
...
auto log_level_label(LogLevel level) -> const char * {
    static const char *level_strings[] = {
        "",     "TRACE", "DEBUG", "INFO",
        "WARN", "ERROR", "FATAL", ""}; // first and last are for All and None metalevels
    return level_strings[static_cast<int>(level)];
}

auto log_level_color(LogLevel level) -> const char * {
    static const char *level_colors[] = {
        "",         "\x1b[34m", "\x1b[36m", "\x1b[32m",
        "\x1b[33m", "\x1b[31m", "\x1b[35m", ""}; // first and last are for All and None metalevels
